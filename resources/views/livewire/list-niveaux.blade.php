    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div class="flex flex-col md:flex-row gap-4 w-full md:w-auto">
            <div class="relative w-full md:w-64">
                ...
                <input type="text" wire:model.live="search" placeholder="Rechercher un niveau..." 
            ...
            <x-button href="{{ route('niveaux.create_level') }}" icon="add" color="indigo">
                Ajouter un niveau
            </x-button>
        </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead>
                <tr class="bg-gray-50">
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-poppins-medium text-gray-500 uppercase tracking-wider">Code</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-poppins-medium text-gray-500 uppercase tracking-wider">Libellé</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-poppins-medium text-gray-500 uppercase tracking-wider">Scolarité</th>
                    <th scope="col" class="px-6 py-3 text-center text-xs font-poppins-medium text-gray-500 uppercase tracking-wider">Classes</th>
                ...
                @forelse ($levelList as $item)
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2 py-1 inline-flex text-xs leading-5 font-poppins-medium rounded-full bg-blue-100 text-blue-800">
                                {{ $item->code }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-poppins text-gray-900">{{ number_format($item->scolarite, 0, ',', ' ') }} FCFA</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <span class="px-2 py-1 inline-flex items-center text-xs leading-5 font-poppins-medium rounded-full {{ $item->classCount > 0 ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-800' }}">
                                <x-icons name="class" class="mr-1" size="xs" /> {{ $item->classCount ?? 0 }} classes
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                            <x-action-menu id="level-{{ $item->id }}">
                                <x-action-menu-item href="{{ route('niveaux.update_level', $item) }}" icon="edit" color="indigo">
                                    Modifier
                                </x-action-menu-item>
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-10 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-gray-500 font-poppins-medium">Aucun niveau trouvé</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
